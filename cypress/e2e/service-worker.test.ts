...
describe("Service Worker", () => {
  ...
  beforeEach(() => {
    cy.intercept({ hostname: "www.google-analytics.com" }, (req) => {
      ...
      if (
        ...
        if (
          req.query["en"] === "Not Installed" ||
          body.includes("Not%20Installed")
        ) {
          req.alias = "NotInstalled";
        } else if (
          req.query["en"] === "Cache Hit" ||
          body.includes("Cache%20Hit")
        ) {
          req.alias = "CacheHit";
        } else if (
          req.query["en"] === "Cache Miss" ||
          body.includes("Cache%20Miss")
        ) {
          req.alias = "CacheMiss";
        }
      }
  ...
  it("installs a ServiceWorker", () => {
    cy.visit("/", { serviceWorker: true })
      ...
      .and((win) => {
        expect(win.navigator.serviceWorker.controller?.state).to.equal(
          "activated"
        );
      });
  });
