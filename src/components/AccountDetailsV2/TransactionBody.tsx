...
const FormattedCurrencyAmount = ({
  ...
  return currency ? (
    <HighlightText>
      {formatAmount(rawAmount, currency.decimals, /* sigFigs= */ 6)}{" "}
      {currency.symbol}
    </HighlightText>
  ) : null;
};
    ...
    <BodyWrap>
      {uniAmountRaw && (
        <>
          ...
          <Trans>for</Trans>{" "}
          <HighlightText>{ENSName ?? shortenAddress(recipient)}</HighlightText>
        </>
      )}{" "}
      <FailedText transactionState={transactionState} />
    </BodyWrap>
...
const TransactionBody = ({
  ...
  switch (info.type) {
    ...
    case TransactionType.COLLECT_FEES:
      return (
        <CollectFeesSummary info={info} transactionState={transactionState} />
      );
    case TransactionType.APPROVAL:
      return (
        <ApprovalSummary info={info} transactionState={transactionState} />
      );
