...
const recentlySearchedAssetsAtom = atomWithStorage<RecentlySearchedAsset[]>(
  "recentlySearchedAssets",
  []
);

export function useAddRecentlySearchedAsset() {
  const [searchHistory, updateSearchHistory] = useAtom(
  ...
  return useCallback(
    (asset: RecentlySearchedAsset) => {
      ...
      const newHistory = searchHistory.filter(
        (oldAsset) =>
          !(
            oldAsset.address === asset.address && oldAsset.chain === asset.chain
          )
      );
...
export function useRecentlySearchedAssets() {
  ...
  const data = useMemo(() => {
    ...
    const queryCollections = queryData?.nftCollections?.edges.map(
      (edge) => edge.node as NonNullable<NftCollection>
    );
    ...
    collections?.forEach(
      (collection) => (resultsMap[collection.address] = collection)
    );
    ...
    shortenedHistory.forEach((asset) => {
      if (asset.address === "NATIVE") {
        ...
        const native = nativeOnChain(
          CHAIN_NAME_TO_CHAIN_ID[asset.chain] ?? SupportedChainId.MAINNET
        );
        const queryAddress =
          getQueryAddress(asset.chain)?.toLowerCase() ??
          `NATIVE-${asset.chain}`;
