...
interface SellAssetState {
  ...
  removeMarketplaceWarning: (
    asset: WalletAsset,
    warning: ListingWarning,
    setGlobalOverride?: boolean
  ) => void;
    ...
    (set) => ({
      ...
      removeMarketplaceWarning: (asset, warning, setGlobalOverride?) => {
        set(({ sellAssets }) => {
          const assetsCopy = [...sellAssets];
          if (
            asset.listingWarnings === undefined ||
            asset.newListings === undefined
          )
            return { sellAssets: assetsCopy };
          const warningIndex =
            asset.listingWarnings?.findIndex(
              (n) => n.marketplace.name === warning.marketplace.name
            ) ?? -1;
          asset.listingWarnings?.splice(warningIndex, 1);
          if (warning?.message?.includes("LISTING BELOW FLOOR")) {
            if (setGlobalOverride) {
              asset.newListings?.forEach(
                (listing) => (listing.overrideFloorPrice = true)
              );
            } else {
              const listingIndex =
                asset.newListings?.findIndex(
                  (n) => n.marketplace.name === warning.marketplace.name
                ) ?? -1;
