      ...
      set(() => {
        const updatedListings = listings.map((listing) => {
          const oldListing = get().listings.find(
            (oldListing) =>
              oldListing.asset.asset_contract.address ===
                listing.asset.asset_contract.address &&
          ...
          const status = () => {
            switch (oldStatus) {
              ...
              case ListingStatus.FAILED:
                return listing.status === ListingStatus.SIGNING
      ...
      set(() => {
        const updatedCollections = collections.map((collection) => {
          const oldCollection = get().collectionsRequiringApproval.find(
            (oldCollection) =>
              oldCollection.collectionAddress ===
          ...
          const status = () => {
            switch (oldStatus) {
              case ListingStatus.APPROVED:
                return ListingStatus.APPROVED;
              case ListingStatus.FAILED:
                return collection.status === ListingStatus.SIGNING
                  ? ListingStatus.SIGNING
                  : ListingStatus.FAILED;
              case ListingStatus.REJECTED:
                return collection.status === ListingStatus.SIGNING
