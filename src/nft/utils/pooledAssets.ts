...
const getPoolParameters = (
  ...
  return {
    delta: protocolParameters?.delta
      ? (protocolParameters.delta as string)
      : undefined,
    fee: protocolParameters?.ammFeeFixed
      ? (protocolParameters.ammFeeFixed as string)
      : undefined,
    spotPrice: (protocolParameters as Record<string, { spotPrice?: string }>)
      ?.poolMetadata?.spotPrice,
    bondingCurve: (
      protocolParameters as Record<string, { bondingCurve?: BondingCurve }>
    )?.poolMetadata?.bondingCurve,
  };
};

const calculateScaledPrice = (
  currentPrice: BigNumber,
  poolFee: BigNumber
): BigNumber => {
  const protocolFee = currentPrice
    .mul(PROTOCOL_FEE_MULTIPLIER)
...
const calcSudoSwapLinearBondingCurve = (
  currentPrice: BigNumber,
  delta: BigNumber,
  position = 0
): BigNumber => {
...
const calcSudoSwapExponentialBondingCurve = (
  currentPrice: BigNumber,
  delta: BigNumber,
  position = 0
): BigNumber => {
...
const calcSudoSwapXykBondingCurve = (
  ...
  if (virtualNFTBalance.sub(BigNumber.from(1)).gt(BigNumber.from(0))) {
    currentPrice = virtualTokenBalance.div(
      virtualNFTBalance.sub(BigNumber.from(1))
    );
  } else {
    return undefined;
  }
...
export const calcSudoSwapPrice = (
  asset: GenieAsset,
  position = 0
): string | undefined => {
  ...
  if (
    !sudoSwapPool.fee ||
    !sudoSwapPool.delta ||
    !sudoSwapPool.spotPrice ||
    !sudoSwapPool.bondingCurve
  )
    return undefined;
  ...
  if (sudoSwapPool.bondingCurve === BondingCurve.Linear) {
    currentPrice = calcSudoSwapLinearBondingCurve(
      currentPrice,
      delta,
      position
    );
  } else if (sudoSwapPool.bondingCurve === BondingCurve.Exponential) {
    currentPrice = calcSudoSwapExponentialBondingCurve(
      currentPrice,
      delta,
      position
    );
  } else if (sudoSwapPool.bondingCurve === BondingCurve.Xyk) {
    const xykCurrentPrice = calcSudoSwapXykBondingCurve(
      currentPrice,
      sudoSwapPool,
      position
    );
...
const calcAmmBasedPoolprice = (asset: GenieAsset, position = 0): string => {
  ...
  const ammFee = nft?.ammFeePercent
    ? (100 + (nft.ammFeePercent as number)) * 100
    : 110 * 100;
...
export const calcAvgGroupPoolPrice = (
  asset: GenieAsset,
...
const recalculatePooledAssetPrice = (
  asset: GenieAsset,
  position: number
): string => {
...
export const recalculateBagUsingPooledAssets = (
  uncheckedItemsInBag: BagItem[]
) => {
  if (
    !uncheckedItemsInBag.some(
      (item) => item.asset.marketplace && isPooledMarket(item.asset.marketplace)
    ) ||
    uncheckedItemsInBag.every(
      (item) =>
        item.status === BagItemStatus.REVIEWED ||
        item.status === BagItemStatus.REVIEWING_PRICE_CHANGE
    )
  )
      ...
      if (isPooledMarket(item.asset.marketplace)) {
        ...
        const itemsInPool =
          asset.marketplace === Markets.Sudoswap
            ? itemsInBag.filter((bagItem) =>
                isInSameSudoSwapPool(item.asset, bagItem.asset)
              )
            : itemsInBag.filter((bagItem) =>
                isInSameMarketplaceCollection(item.asset, bagItem.asset)
              );
        const calculatedPrice = isPriceChangedAsset
          ? calcAvgGroupPoolPrice(asset, itemsInPool.length)
          : recalculatePooledAssetPrice(
              asset,
              itemsInPool.findIndex(
                (itemInPool) => itemInPool.asset.tokenId === asset.tokenId
              )
            );

        if (isPriceChangedAsset && item.asset.updatedPriceInfo)
          item.asset.updatedPriceInfo.ETHPrice =
            item.asset.updatedPriceInfo.basePrice = calculatedPrice;
