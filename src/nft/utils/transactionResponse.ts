...
export const parseTransactionResponse = (
  transactionResponse: TxResponse | undefined,
  ethPrice: number
) => {
  ...
  if (transactionResponse !== undefined) {
    ...
    if (nftsPurchased && nftsNotPurchased && txReceipt) {
      ...
      totalUSDRefund =
        totalRefundValue &&
        parseFloat(formatEther(totalRefundValue)) * ethPrice;
...
export const getSuccessfulImageSize = (
  ...
  if (numSuccessful === 1) {
    ...
    numSuccessful === 4 ||
    (numSuccessful >= 7 && numSuccessful < 13)
  ) {
