...
export async function approveCollectionRow(
  ...
  if (
    (collectionRow.status === ListingStatus.REJECTED ||
      collectionRow.status === ListingStatus.FAILED) &&
    pauseAllRows
  )
    pauseAllRows();
}
...
export const getListingState = (
  ...
  for (const collection of collectionsRequiringApproval) {
    if (collection.status !== ListingStatus.PENDING)
      allCollectionsPending = false;
    if (collection.status !== ListingStatus.DEFINED)
      allCollectionsDefined = false;
    if (collection.status === ListingStatus.SIGNING) anyActiveSigning = true;
    else if (collection.status === ListingStatus.FAILED)
      anyActiveFailures = true;
    else if (collection.status === ListingStatus.REJECTED)
      anyActiveRejections = true;
