...
export const ListingButton = ({
  ...
  const [
    ...
    const noMarketplacesSelected = sellAssets.some(
      (asset: WalletAsset) => asset.marketplaces === undefined
    );
    const missingExpiration = sellAssets.some((asset) => {
      return (
        asset.expirationTime != null &&
        asset.expirationTime * 1000 - Date.now() < ms`60 seconds`
      );
    });
    ...
    const overMaxExpiration = sellAssets.some((asset) => {
      return (
        asset.expirationTime != null &&
        asset.expirationTime - Date.now() > ms`180 days`
      );
    });
    ...
    for (const asset of sellAssets) {
      if (asset.newListings) {
        for (const listing of asset.newListings) {
          ...
          else if (isNaN(listing.price) || listing.price < 0)
  ...
  const [disableListButton, warningMessage] = useMemo(() => {
    ...
    const warningMessage = noMarketplacesSelected
      ...
      : listingsMissingPrice.length > 0
      ? `${listingsMissingPrice.length} item price${pluralize(
          listingsMissingPrice.length
        )} not set`
      : invalidPrices.length > 0
      ? `${invalidPrices.length} price${pluralize(
          invalidPrices.length
        )} are invalid`
      : listingsBelowFloor.length > 0
      ? `${listingsBelowFloor.length} item${pluralize(
          listingsBelowFloor.length
        )} listed below floor`
      : listingsAboveSellOrderFloor.length > 0
      ? `${listingsAboveSellOrderFloor.length} item${pluralize(
          listingsAboveSellOrderFloor.length
        )} already listed`
      : "";
      ...
      <Box position="relative" width="full">
        ...
        <Box
          ...
          cursor={
            ...
            ].includes(listingStatus) || disableListButton
              ? "default"
              : "pointer"
          }
          className={styles.button}
          onClick={() =>
            listingStatus !== ListingStatus.APPROVED && warningWrappedClick()
          }
          type="button"
          style={{
            color: showResolveIssues
              ? theme.accentTextDarkPrimary
              : theme.white,
            opacity:
              ![
                ListingStatus.DEFINED,
                ListingStatus.FAILED,
                ListingStatus.CONTINUE,
