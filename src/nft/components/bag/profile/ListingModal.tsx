...
const ListingModal = () => {
  ...
  useEffect(() => {
    useNFTList.subscribe(
      (state) => (looksRareNonceRef.current = state.looksRareNonce)
    );
  }, []);
      ...
      setAllCollectionsApproved(
        collectionsRequiringApproval.every(
          (collection: CollectionRow) =>
            collection.status === ListingStatus.APPROVED
        )
      );
    if (
  ...
  const allCollectionsApprovedOrPaused = useMemo(
    () =>
      collectionsRequiringApproval.every(
        (collection: CollectionRow) =>
          collection.status === ListingStatus.APPROVED ||
          collection.status === ListingStatus.PAUSED
      ),
    [collectionsRequiringApproval]
  );
  ...
  useEffect(() => {
    if (
      listingStatus === ListingStatus.SIGNING &&
      allCollectionsApprovedOrPaused &&
      allListingsApprovedOrPaused
    ) {
      resetAllRows();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const startListingFlow = async () => {
    ...
    if (
      !collectionsRequiringApproval?.some(
        (collection) => collection.status === ListingStatus.PAUSED
      )
    ) {
      setListingStatus(ListingStatus.SIGNING);
      setOpenIndex(1);
  ...
  const signListings = async () => {
    ...
    const allListingsSigned = listings.every(
      (listing: ListingRow) => listing.status === ListingStatus.APPROVED
    );
    const paused = listings.some(
      (listing: ListingRow) => listing.status === ListingStatus.PAUSED
    );
  ...
  const pauseAllRows = () => {
    for (const collection of collectionsRequiringApproval) {
      pauseRow(
        collection,
        collectionsRequiringApproval,
        setCollectionsRequiringApproval as Dispatch<AssetRow[]>
      );
    }
    for (const listing of listings) {
      ...
      >
        ...
        >
          {showSuccessScreen ? (
            ...
            >
              <ListingSection
                sectionTitle={`Listed ${listings.length} item${pluralize(
                  listings.length
                )} for sale`}
                rows={listings}
                index={0}
            ...
            <>
              <ListingSection
                sectionTitle={`Approve ${
                  collectionsRequiringApproval.length
                } collection${pluralize(collectionsRequiringApproval.length)}`}
                title="COLLECTIONS"
              ...
              />
              <ListingSection
                sectionTitle={`Confirm ${listings.length} listing${pluralize(
                  listings.length
                )}`}
