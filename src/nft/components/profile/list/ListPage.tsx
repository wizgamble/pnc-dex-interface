...
const Overlay = styled.div`
  ...
  width: 100vw;
  background: ${({ theme }) =>
    `linear-gradient(180deg, rgba(0, 0, 0, 0) 0%, ${theme.backgroundBackdrop} 100%)`};
`;
...
const EthValueWrapper = styled.span<{ totalEthListingValue: boolean }>`
  ...
  color: ${({ theme, totalEthListingValue }) =>
    totalEthListingValue ? theme.textPrimary : theme.textSecondary};
...
export const ListPage = () => {
  ...
  const anyListingsMissingPrice = useMemo(
    () => !!listings.find((listing) => !listing.price),
    [listings]
  );
  ...
  useEffect(() => {
    ...
    if (state.allListingsApproved) setListingStatus(ListingStatus.APPROVED);
      ...
      state.anyPaused &&
      !state.anyActiveFailures &&
      !state.anyActiveSigning &&
      !state.anyActiveRejections
    ) {
      ...
      state.allListingsPending ||
      (state.allCollectionsPending && state.allListingsDefined)
    )
      setListingStatus(ListingStatus.PENDING);
    else if (state.anyActiveFailures && listingStatus !== ListingStatus.PAUSED)
        ...
        <ListingHeader>
          <TitleWrapper>
            <BackArrowIcon
              ...
              cursor="pointer"
            />
            <div className={isMobile ? headlineSmall : headlineLarge}>
              Sell NFTs
            </div>
          </TitleWrapper>
          <ButtonsWrapper>
            ...
            <ProceedsAndButtonWrapper>
              <ProceedsWrapper>
                <EthValueWrapper totalEthListingValue={!!totalEthListingValue}>
                  {totalEthListingValue > 0
                    ? formatEth(totalEthListingValue)
                    : "-"}{" "}
                  ETH
                </EthValueWrapper>
              ...
              <ListingButtonWrapper>
                <ListingButton
                  ...
                  buttonText={
                    anyListingsMissingPrice && !isMobile
                      ? t`Set prices to continue`
                      : t`Start listing`
                  }
