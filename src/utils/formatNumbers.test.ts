...
describe("currencyAmountToPreciseFloat", () => {
  it("small number", () => {
    const currencyAmount = CurrencyAmount.fromFractionalAmount(
      USDC_MAINNET,
      "20000",
      "7"
    );
  ...
  it("tiny number", () => {
    ...
    expect(currencyAmountToPreciseFloat(currencyAmount)).toEqual(
      0.000000285714
    );
  });
  it("lots of decimals", () => {
...
describe("formatDollar for a price", () => {
  ...
  it("< 0.000001", () => {
    expect(formatDollar({ num: 0.00000000011231231432, isPrice })).toEqual(
      "$1.12e-10"
    );
  });
  ...
  it("lessPreciseStablecoinValues number less than 1, rounds to 0.999", () => {
    expect(
      formatDollar({ num: 0.9994, isPrice, lessPreciseStablecoinValues: true })
    ).toEqual("$0.999");
  });
  it("lessPreciseStablecoinValues number less than, rounds to 1.00", () => {
    expect(
      formatDollar({ num: 0.9995, isPrice, lessPreciseStablecoinValues: true })
    ).toEqual("$1.00");
  });
