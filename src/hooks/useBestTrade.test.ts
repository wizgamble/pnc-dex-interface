...
const mockUseAutoRouterSupported =
  useAutoRouterSupported as jest.MockedFunction<typeof useAutoRouterSupported>;
const mockUseIsWindowVisible = useIsWindowVisible as jest.MockedFunction<
  typeof useIsWindowVisible
>;

const mockUseRoutingAPITrade = useRoutingAPITrade as jest.MockedFunction<
  typeof useRoutingAPITrade
>;
const mockUseClientSideRouter = useClientSideRouter as jest.MockedFunction<
  typeof useClientSideRouter
>;
const mockUseClientSideV3Trade = useClientSideV3Trade as jest.MockedFunction<
  typeof useClientSideV3Trade
>;
...
describe("#useBestV3Trade ExactIn", () => {
  it("does not compute routing api trade when routing API is not supported", async () => {
    ...
    const { result } = renderHook(() =>
      useBestTrade(TradeType.EXACT_INPUT, USDCAmount, DAI)
    );
    ...
    expect(mockUseClientSideV3Trade).toHaveBeenCalledWith(
      TradeType.EXACT_INPUT,
      USDCAmount,
      DAI
    );
...
describe("#useBestV3Trade ExactOut", () => {
  it("does not compute routing api trade when routing API is not supported", () => {
    ...
    const { result } = renderHook(() =>
      useBestTrade(TradeType.EXACT_OUTPUT, DAIAmount, USDC_MAINNET)
    );
